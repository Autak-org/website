...
export function SiteHeader({ locale, dictionary, projectNav }: Props) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 pt-[env(safe-area-inset-top,0px)]">
      {/* Backdrop lives on the bar, not the header: filter creates a containing
          block that would otherwise trap the fixed mobile panel. */}
      <div
        className={cn(
          "border-b transition-colors duration-300",
          solid
            ? "border-ink-200 bg-white/95 backdrop-blur-xl"
            : "border-white/10 bg-transparent",
        )}
      >
        <div className="mx-auto flex h-[var(--header-bar)] w-full max-w-7xl items-center gap-2 px-4 sm:gap-4 sm:px-6 lg:px-10">
          <Link
      ...
      {open ? (
        <div
          id="mobile-menu"
          ref={panelRef}
          className="fixed inset-x-0 top-header bottom-0 z-40 overflow-y-auto overscroll-contain bg-white lg:hidden"
        >
          <nav
            aria-label={dictionary.a11y.mainNav}
            className="mx-auto w-full max-w-7xl px-4 pt-4 pb-[max(5rem,env(safe-area-inset-bottom,0px))] sm:px-6"
          >
                ...
                (link) => (
                  <li key={link.href} className="border-b border-ink-200">
                    <Link
                      href={link.href}
                      onClick={close}
                      className="flex min-h-12 items-center py-3.5 text-xl font-semibold text-ink-900 transition-colors hover:text-brand-600"
                    >
                      {link.label}
                    </Link>
                  </li>
...
function MenuLink({
  ...
  const className = "group block px-4 py-3.5 transition-colors hover:bg-brand-50";
...
function MobileGroup({
  ...
  return (
    <section className="mt-8">
      <h2 className="text-xs font-semibold tracking-[0.16em] text-ink-500 uppercase">
        {title}
      </h2>
      <ul className="mt-3 grid gap-2">
        {links.map((link) => (
          <li key={link.href} className="overflow-hidden rounded-2xl border border-ink-200">
        ...
        {locales.map((item) => (
          <li key={item}>
            <Link
              ...
              className={cn(
                "flex min-h-12 items-center justify-between gap-2 rounded-2xl border px-3 py-3 text-sm font-medium",
                item === locale
            ...
            >
              <span className="min-w-0">
                <span className="block truncate">{localeNames[item]}</span>
  ...
  return (
    <div ref={wrapperRef} className="relative">
      <button
        ...
        onClick={() => setOpen((value) => !value)}
        className={cn(
          "inline-flex items-center gap-1.5 rounded-full px-2.5 py-2 text-sm font-semibold whitespace-nowrap transition-colors sm:px-3",
          solid
            ...
            {locales.map((item) => (
              <li key={item}>
                <Link
                  ...
                  aria-current={item === locale ? "true" : undefined}
                  className="flex min-h-11 items-center justify-between gap-3 px-4 py-3 text-sm transition-colors hover:bg-brand-50"
                >
                  <span className="min-w-0">
                    <span
                      className={cn(
